...
if [[ "$OWN_DEVICE" == "N" ]]; then
    ...
    sed -i "s/\$VIDEO_INPUT_FOLDER_ON_DEVICE/\/home\/edgeuser\/samples\/input/" $DEPLOYMENT_MANIFEST_FILE
...
##################################################################################################################################################    
    ...
    sed -i "s/\$VIDEO_INPUT_FOLDER_ON_DEVICE/\/home\/edgeuser\/samples\/input/" $BYOD_DEPLOYMENT_MANIFEST_FILE
